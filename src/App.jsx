...
const Login = ({ onLogin }) => {
  ...
  const handleLogin = (e) => {
    ...
    if (role === 'admin' && username === 'admin' && password === 'admin123') {
      ...
      onLogin({ username, role: 'faculty' });
    } else {
      setError('Invalid credentials');
    }
  };

  const bgClass = role === 'admin' ? 'bg-blue-50' : 'bg-green-50';
  const btnClass = role === 'admin' ? 'bg-blue-900 hover:bg-blue-800' : 'bg-green-900 hover:bg-green-800';
  const accentClass = role === 'admin' ? 'accent-blue-600' : 'accent-green-600';

  return (
    <div className={`min-h-screen flex items-center justify-center ${bgClass}`}>
      <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md">
        <div className="flex justify-center mb-4">
          <div className="w-20 h-20 bg-blue-900 rounded-full flex items-center justify-center p-2">
            <img src="/college_logo.png" alt="Logo" className="w-full h-full object-contain bg-white rounded-full" />
          </div>
        </div>
        <h2 className="text-2xl font-bold text-center text-blue-900 mb-2">Smart Classroom System</h2>
        {/* Role selector */}
        <div className="flex justify-center mb-2 space-x-4">
          <label className="flex items-center space-x-1">
            <input type="radio" name="role" value="admin" checked={role === 'admin'} onChange={() => setRole('admin')} className={accentClass} />
            <span className="text-sm font-medium text-slate-700">Admin</span>
          </label>
          <label className="flex items-center space-x-1">
            <input type="radio" name="role" value="faculty" checked={role === 'faculty'} onChange={() => setRole('faculty')} className={accentClass} />
            <span className="text-sm font-medium text-slate-700">Faculty</span>
          </label>
        </div>
        {error && <div className="bg-red-50 text-red-600 p-3 rounded-lg mb-2 text-sm text-center">{error}</div>}
        <form onSubmit={handleLogin} className="space-y-2">
          <div>
            <label className="block text-sm font-bold text-slate-600 mb-1">Username</label>
            <input type="text" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter username" />
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-600 mb-1">Password</label>
            <input type="password" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" />
          </div>
          <button type="submit" className={`w-full ${btnClass} text-white py-3 rounded-lg font-bold transition shadow-md`}>
            Login as {role === 'admin' ? 'Admin' : 'Faculty'}
          </button>
        </form>
...
const SmartClassroomSystem = () => {
  ...
  const [classrooms, setClassrooms] = useState([
    ...
    { name: 'Software Engineering Lab', type: 'Lab' },
    { name: 'Open Source Lab', type: 'Lab' },
    { name: 'DataBase Lab', type: 'Lab' },
    { name: 'Internet Technology Lab', type: 'Lab' }
  ]);
  ...
  const [currentTT, setCurrentTT] = useState({
    ...
    courses: [],
    schedule: {},
    labRoomAssignments: {} // Maps "day-period" to lab room name
  });
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (ttData) { setTimetables(Array.isArray(ttData) ? ttData : []); }
        if (bookData) { setFacultyBookings(Array.isArray(bookData) ? bookData : []); }
        if (allocData) { setAllocations(Array.isArray(allocData) ? allocData : []); }
        if (roomData) { setClassrooms(Array.isArray(roomData) ? roomData : []); }
        if (extData) { setExternalTimetables(Array.isArray(extData) ? extData : []); }
      } catch (e) {
      ...
      timetables.forEach(tt => {
        ...
        sessions.forEach(session => {
          ...
          subjects.forEach(sub => {
            ...
            const isLabSession = cleanSub.includes('(L)');

            // Priority 1: For lab sessions, use assigned Lab Resource
            // Priority 2: Default Classroom (for non-lab sessions or if no lab resource assigned)
            let targetRoom = '';
            if (isLabSession && course.labResource) {
              targetRoom = course.labResource;
            } else if (!isLabSession) {
              targetRoom = tt.classroom;
            } else {
              // Lab session but no lab resource assigned - use default classroom as fallback
              targetRoom = tt.classroom;
            }
            ...
            if (targetRoom) {
              ...
              if (isRoomFree) {
                ...
                session.periods.forEach(p => {
                  ...
                  addToOccupied(session.day, p.t, targetRoom, {
                    type: isLabSession ? 'Lab Session' : 'Class',
                    subject: cleanSub
                  }, allocId);
                  newAllocations.push({
                    id: allocId,
                    type: isBatchSplit ? 'Batch Split' : (isLabSession ? 'Lab Session' : 'Class'),
                    day: session.day,
                    time: p.t,
                    ...
                    isLabSession,
                    sessionDuration: session.periods.length
                  });
                });
              } else {
                // Conflict
                ...
                newConflicts.push({
                  ...
                  msg: `${isLabSession ? 'Lab' : 'Class'} Block Conflict: ${cleanSub} (${session.periods.length} periods)`,
                  day: session.day,
                  time: `${firstP.t}..`,
                  classroom: targetRoom,
                  collidingAllocationId: conflictingAlloc?.allocId,
                  type: isLabSession ? 'Lab Session' : 'Student Class',
                  subject: cleanSub,
                  sugg: `Room ${targetRoom} occupied during ${session.periods.length}-period block.`
                });
              }
            } else {
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      <div className="max-w-[1600px] mx-auto p-4 print:p-0 print:max-w-none">
        ...
        <header className="bg-blue-900 text-white rounded-xl shadow-lg p-6 mb-6 flex items-center justify-between print:hidden">
          ...
          <div className="text-right flex flex-col items-end gap-2">
            <div className="flex items-center gap-3">
              <span className="bg-blue-800 px-3 py-1 rounded-full text-sm font-semibold capitalize flex items-center gap-2">
                <Users className="w-4 h-4" /> {user.username} ({user.role})
              </span>
              <button
                onClick={() => setUser(null)}
                className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg text-sm font-bold transition"
              >
                Logout
              </button>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm mb-6 flex overflow-hidden border border-slate-200 print:hidden overflow-x-auto">
          {[
            { id: 'resources', label: 'Resource Manager', icon: <Settings className="w-5 h-5" />, roles: ['admin'] },
            { id: 'generator', label: 'Timetable Generator', icon: <Calendar className="w-5 h-5" />, roles: ['admin', 'faculty'] },
            { id: 'faculty', label: 'Faculty Booking', icon: <Users className="w-5 h-5" />, roles: ['admin', 'faculty'] },
            { id: 'allocate', label: 'Allocations', icon: <CheckCircle className="w-5 h-5" />, roles: ['admin'] },
            { id: 'results', label: 'View Results', icon: <LayoutGrid className="w-5 h-5" />, roles: ['admin', 'faculty'] },
            { id: 'unallocated', label: 'Free Resources', icon: <BookOpen className="w-5 h-5" />, roles: ['admin'] }
          ]
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6 print:hidden">
                  ...
                  {/* Lab Room Assignments Section */}
                  <div className="space-y-4 bg-purple-50 p-4 rounded-lg border border-purple-200">
                    <h3 className="font-bold text-lg text-purple-900 border-b pb-2">Lab Room Assignments</h3>
                    {(() => {
                      // Detect all lab sessions from schedule
                      const labSessions = [];
                      days.forEach(day => {
                        timeSlots.forEach(slot => {
                          const subject = currentTT.schedule[day]?.[slot.p];
                          if (subject && subject.includes('(L)')) {
                            labSessions.push({
                              key: `${day}-${slot.p}`,
                              day,
                              period: slot.p,
                      ...
                      // Group by unique subject
                      const uniqueSubjects = [...new Set(labSessions.map(s => s.subject))];

                      return uniqueSubjects.length > 0 ? (
                        <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                          {uniqueSubjects.map(subject => {
                            // Find all specific sessions for this subject
                            const subjectSessions = labSessions.filter(s => s.subject === subject);
                            // Get currently assigned lab (check the first session)
                            const currentLab = currentTT.labRoomAssignments[subjectSessions[0].key] || '';

                            return (
                              <div key={subject} className="bg-white p-2 rounded shadow-sm border border-purple-100 flex items-center gap-2">
                                <div className="flex-1 flex flex-col justify-center">
                                  <span className="font-bold text-blue-900 text-sm">{subject}</span>
                                  <span className="text-[10px] text-gray-500">
                                    {subjectSessions.length} session{subjectSessions.length !== 1 ? 's' : ''}: {subjectSessions.map(s => `${s.day} P${s.period}`).join(', ')}
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  ...
                                  <select
                                    ...
                                    onChange={e => {
                                      ...
                                      // Update ALL sessions for this subject
                                      const newAssignments = { ...currentTT.labRoomAssignments };
                                      subjectSessions.forEach(s => {
                                        newAssignments[s.key] = newLab;
                                      });

                                      setCurrentTT(prev => ({
                                        ...prev,
                                        labRoomAssignments: newAssignments
                                      }));
                                    }}
                                  >
                                    <option value="">-- Select Lab --</option>
                                    {classrooms.filter(r => r.type === 'Lab').map(r => (
                        ...
                        <div className="text-center text-gray-400 italic text-xs py-4">
                          No lab sessions detected. Enter subjects with (L) suffix in the schedule.
                        </div>
                      );
                    })()}
                  </div>
